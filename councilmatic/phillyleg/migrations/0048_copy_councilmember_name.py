...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        CouncilMember = orm['phillyleg.CouncilMember']
        CouncilMemberAlias = orm['phillyleg.CouncilMemberAlias']

        for member in CouncilMember.objects.all():
            member.aliases.add(CouncilMemberAlias(name=member.name))
    ...
    def backwards(self, orm):
        ...
        # It's not worth it to do this one right.
        CouncilMember = orm['phillyleg.CouncilMember']
        CouncilMemberAlias = orm['phillyleg.CouncilMemberAlias']
        CouncilMemberTenure = orm['phillyleg.CouncilMemberTenure']

        for member in CouncilMember.objects.all():
            current_member = member
            for name in member.aliases.all():
                current_member.name = name
                current_member.save()

                # Create a new CouncilMember model instance for the next name.
                # This only matters if there's more than one name:
                current_member = CouncilMember(
