...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'MetaData_Location.matched_text'
        db.add_column(u'phillyleg_metadata_location', 'matched_text',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=2048),
                      keep_default=False)

        # Copying data from address to matched_text
        for location in orm.MetaData_Location.objects.all():
            location.matched_text = location.address
            location.save()

        # Removing unique constraint on 'MetaData_Location', fields ['address']
        db.delete_unique(u'phillyleg_metadata_location', ['address'])

        # Adding unique constraint on 'MetaData_Location', fields ['matched_text']
        db.create_unique(u'phillyleg_metadata_location', ['matched_text'])

    def backwards(self, orm):
        # Copying data from matched_text to address
        for location in orm.MetaData_Location.objects.all():
            location.address = location.matched_text
            location.save()

        # Removing unique constraint on 'MetaData_Location', fields ['matched_text']
        db.delete_unique(u'phillyleg_metadata_location', ['matched_text'])

        # Adding unique constraint on 'MetaData_Location', fields ['address']
        db.create_unique(u'phillyleg_metadata_location', ['address'])

        # Deleting field 'MetaData_Location.matched_text'
        db.delete_column(u'phillyleg_metadata_location', 'matched_text')
    ...
    models = {
        ...
        u'phillyleg.councilmemberalias': {
            'Meta': {'object_name': 'CouncilMemberAlias'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'member': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'aliases'", 'to': u"orm['phillyleg.CouncilMember']"}),
        ...
        u'phillyleg.metadata_location': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'matched_text': ('django.db.models.fields.CharField', [], {'default': "''", 'max_length': '2048'}),
            'updated_datetime': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
